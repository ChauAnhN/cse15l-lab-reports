...
## **Part 3**
Prior to week 2, I did not know how to create a web server out of my code. As a result, the lab session that week taught me new knowledge I've never been exposed to before. To be specific, I didn't know about the methods `.getPath()` and prior to CSE15L I was unfamiliar with what each character in a URL might mean. (For example, `?` stands for query and can be used to run certain code actions.) As a result, I learned that simply splitting the URL into different key words that follow a query, I can call upon different methods within my code. 
